...
Install the current development version from GitHub (repository: <https://github.com/wep69/pcaLab>).

Install **without the vignettes** (fastest; only the mandatory dependencies are required):

```r
# install.packages("remotes")  # run once if remotes is not installed
remotes::install_github("wep69/pcaLab")
```

Install **with the vignettes** built on your machine:

```r
remotes::install_github("wep69/pcaLab", build_vignettes = TRUE)
```

The `build_vignettes = TRUE` form additionally requires `knitr`, `rmarkdown`, and the optional engine packages used by the vignettes. After installation, list the available vignettes with `vignette(package = "pcaLab")`.

Alternatively, install from the release tarball:

```r
install.packages("pcaLab_0.1.0.tar.gz", repos = NULL, type = "source")
```

The package requires R (>= 4.2.0). Advanced engines are optional: `pca_capabilities()` reports what is available in the current R installation.

## Cheat sheet and release assets

The one-page cheat sheet `pcaLab_Cheatsheet.pdf` is distributed as a separate file: it is included in this repository under `cheatsheet/` and attached to the GitHub release alongside the source tarball, so it is not part of the installed package.
